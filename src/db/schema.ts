...
export const ROLE_ENUM = pgEnum("role", ["USER", "ADMIN", "OPERATOR"])
...
export const inventoryPurchases = pgTable("inventory_purchases", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow(),
  // New fields for credit purchases
  isPaid: boolean("is_paid").default(true),
  paidAmount: decimal("paid_amount", { precision: 10, scale: 2 }).default("0"),
  status: varchar("status", { length: 20 }).default("PAID"), // PAID, PARTIAL, PENDING
  dueDate: timestamp("due_date", { withTimezone: true }),
  metadata: jsonb("metadata"),
})
